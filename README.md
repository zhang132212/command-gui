...
> **版本** 0.2.0-beta.1 · **目标** Minecraft 26.2 · **加载器** Fabric Loader 0.19.3+ · **许可证** GPL-3.0
...
- **机器开关界面**：
  ...
  - 模式选择界面：点选变黑 + 琥珀待选、单选互斥、检测状态锁定、chip 网格滚动条
...
- **模式**：独立于开关的附加流程，支持单选（互斥）/多选，可配检测方块、独立锁定间隔
- **模式编排**：多模式同时切换时按预设顺序 + 间隔执行（启动链/停止链）
...
2. 客户端 mods 放入：`command-gui-0.2.0-beta.1.jar`
3. 服务端 mods 放入：`command-gui-server-0.2.0-beta.1.jar`
...
- 独立的开机/关机流程对，与开关互不影响
- `singleSelect`（☑ 勾选）：**单选模式互斥** —— 启动新单选模式会自动停掉其他运行中的单选模式（走各自关机流程）
- 未勾选的模式可任意多选并存
- 模式可配 `switchInterval`（0 = 跟随机器）与独立 `detection`
...
./gradlew build
# 产物：
#   build/libs/command-gui-0.2.0-beta.1.jar
#   server/build/libs/command-gui-server-0.2.0-beta.1.jar
```
