...
public abstract class AbstractCommandTab implements Tab {
   ...
   protected void rebuildVisibleCategoryButtons() {
      ...
      if (this.area != null) {
         ...
         for (int i = startIndex; i < endIndex; i++) {
            ...
            int y = this.area.top() + (i - startIndex) * 18;
            btn.setY(y);
   ...
   public boolean isInCategoryArea(double mouseX, double mouseY) {
      return this.area == null
         ...
         : mouseX >= (double)(this.area.left() + this.sidebarOffset())
            && mouseX < (double)(this.area.left() + this.sidebarOffset() + this.categoryTabWidth() + 12 + 2)
   ...
   protected int getCommandAreaLeft() {
      return this.area.left() + this.sidebarOffset() + this.categoryTabWidth() + 12 + 8;
   }

   protected int getCommandAreaWidth() {
      return this.area.width() - this.sidebarOffset() - this.categoryTabWidth() - 12 - 8;
   }
   ...
   protected void rebuildButtons() {
      ...
      if (this.area != null) {
         ...
         int colWidth = commandAreaWidth / 3;
         int y = this.area.top();
         int maxY = this.area.bottom();
         int startIndex = this.scrollOffset * 3;
         int visibleRows = this.area.height() / 24;
         int maxItems = visibleRows * 3;
         ...
         for (int i = 0; i < Math.min(maxItems, count - startIndex); i++) {
            ...
            this.commandButtons.add(this.buildCommandButton(index, btnX + 2, btnY, btnWidth, 22));
            this.onCommandButtonBuilt(index, btnX + 2, btnY, btnWidth, 22);
         }
      }
   }
   ...
   public int getMaxScroll() {
      if (this.area != null && this.getFilteredCommandCount() != 0) {
         int visibleRows = this.area.height() / 24;
         int totalRows = (this.getFilteredCommandCount() + 3 - 1) / 3;
   ...
   public int getVisibleRowCount() {
      ...
      return Math.max(1, this.area.height() / 24);
   }
   ...
   public void renderSeparator(GuiGraphicsExtractor guiGraphics) {
      if (this.area != null) {
         int separatorX = this.area.left() + this.sidebarOffset() + this.categoryTabWidth() + 12 + 4;
   ...
   public void renderCategoryScrollbar(GuiGraphicsExtractor guiGraphics) {
      if (this.area != null) {
         int maxScroll = this.getMaxCategoryScroll();
         int scrollbarX = this.area.left() + this.sidebarOffset() + this.categoryTabWidth() + 2;
         int scrollbarTop = this.area.top();
         int scrollbarHeight = this.area.height();
         ScrollbarHandle handle = new ScrollbarHandle(scrollbarX, scrollbarTop, 12, scrollbarHeight);
