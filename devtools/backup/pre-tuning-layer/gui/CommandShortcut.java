...
public final class CommandShortcut {
    ...
    public static String normalizeCodes(List<Integer> keys) {
        ...
        boolean ctrl = keys.contains(341) || keys.contains(345);
        boolean alt = keys.contains(342) || keys.contains(346);
        boolean shift = keys.contains(340) || keys.contains(344);
        ...
        if (ctrl) {
            sb.append(341).append('+');
        }
        if (alt) {
            sb.append(342).append('+');
        }
        if (shift) {
            sb.append(340).append('+');
        }
    ...
    public static String findConflict(String shortcut, String selfName) {
        ...
        for (CommandConfig.Category category : CommandConfig.getCategories()) {
            for (Map.Entry<String, CommandConfig.CommandEntry> entry : category.commands.entrySet()) {
                if (entry.getKey().equals(selfName) || !normalized.equals(CommandShortcut.normalize(entry.getValue().shortcut))) continue;
    ...
    private static String scanVanillaKeyMappings(List<Integer> keys) {
        ...
        for (KeyMapping mapping : mappings) {
            ...
            for (int key : keys) {
                ...
                if (CommandShortcut.isModifierKey(key) || !mapping.matches(target = InputConstants.Type.KEYSYM.getOrCreate(key))) continue;
                String category = "?";
                try {
